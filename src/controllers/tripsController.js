...
// GET /trips/pending — courses en attente d'un chauffeur (F9/F10 côté app chauffeur)
async function listPending(req, res, next) {
  try {
    ...
    if (vehicle_category) query = query.eq('vehicle_category', vehicle_category);
...
// Version simple MVP : toutes les courses en attente, non filtrées par zone
// (le filtrage géographique fin pourra être ajouté en V2 avec la heatmap).
async function listPendingTrips(req, res, next) {
  try {
    const { data, error } = await supabase
      .from('trips')
      .select('*')
...
module.exports = {
  estimate, createTrip, getTrip, listTrips, listPendingTrips,
